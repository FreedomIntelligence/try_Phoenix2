...
class GPT(nn.Module):
    ...
    def forward(
        ...
        x = deepspeed.checkpointing.checkpoint(self.transformer.wte, idx)
        
        if not use_kv_cache:
            for block in self.transformer.h:
                # x, *_ = block(x, (cos, sin), max_seq_length)
                x, *_ = deepspeed.checkpointing.checkpoint(block, x, (cos, sin), max_seq_length)
        else:
            ...
            for i, block in enumerate(self.transformer.h):
                ...
                x, self.kv_caches[i] = deepspeed.checkpointing.checkpoint(block, x, (cos, sin), max_seq_length, mask, input_pos, self.kv_caches[i])

        # x = self.transformer.ln_f(x)
        x = deepspeed.checkpointing.checkpoint(self.transformer.ln_f, x)
